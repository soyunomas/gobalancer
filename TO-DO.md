...
- [x] **Config Hot-Reload Atómico:** Validación de seguridad antes de recargar configuración con `SIGHUP`.
...
- [ ] **Dynamic Gateway Discovery (DHCP Support):**
    - [ ] *Problema:* Gateways estáticos en `config.toml` rompen si el ISP cambia la IP (muy común en 4G/Starlink).
    - [ ] *Solución:* Permitir `gateway = "auto"`. Usar syscalls para preguntar al Kernel "¿quién es el gateway actual de `eth0`?".
- [ ] **Decision Traceability (Logic Audit):**
    - [ ] *Objetivo:* "Saber el recorrido de la aplicación".
    - [ ] *Implementación:* Añadir un modo `--trace` que loguee la "Matriz de Decisión": por qué se eligió ruta A sobre ruta B (ej: "Ruta A descartada por SLA > 150ms", "Ruta B peso ajustado a 0").
- [ ] **Notification Hooks (Alerting):**
    - [ ] *Necesidad:* Saber si el internet cae sin mirar la consola.
    - [ ] *Implementación:* Ejecutar scripts (`on_change = "./notify.sh"`) o Webhooks (Slack/Discord) ante eventos UP/DOWN.
- [ ] **Runtime Profiling (pprof):**
    - [ ] Exponer servidor HTTP opcional en `localhost:6060/debug/pprof` para ver en tiempo real dónde gasta CPU/RAM la aplicación (flamegraphs).
...
- [ ] **Sticky Sessions (Hash Policy):**
    - [ ] *Problema:* Bancos y HTTPS cierran sesión si la IP pública cambia en cada petición.
    - [ ] *Solución:* Configurar `fib_multipath_hash_policy` del Kernel a L4 (Flujo) para garantizar persistencia de sesión.
- [ ] **DNS Integration:**
